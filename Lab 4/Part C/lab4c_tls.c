...
void handleInput(char* inputoriginal){
    ...
    while (n < len) {
        ...
        if(strncmp(input, "SCALE=", 6) == 0)
        {
            ...
            prin2[17] = '\0';
            int test = SSL_write(secureSL, prin2, strlen(prin2));
            if (test < 0){
...
int main(int argc, char ** argv) {
    ...
    while (1) {
        ...
        switch (ret) {
            ...
            break;
            default:
            fprintf(stderr, "ERROR: Invalid argument. Valid arguments are --period=#, --scale=[C,F], --log=FILENAME, --id=[9 digit number], --hostname=HOSTNAME\n");
            exit(1);
        }
    }
    ...
    sCTX = SSL_CTX_new(TLSv1_client_method());
    if(sCTX == NULL){
        fprintf(stderr, "ERROR: failed to initialize server context\n");
        exit(2);
    }
    secureSL = SSL_new(sCTX);
    test = SSL_set_fd(secureSL, socketFD);
    if (test <= 0) {
        fprintf(stderr, "ERROR: SSL_set_fd() failed\n");
        exit(2);
    } else{
        test = SSL_connect(secureSL);
        if (test == 1){
            char str[10];
            sprintf(str, "ID=%d\n", iflag);
            test = SSL_write(secureSL, str, strlen(str));
            if (test < 0){
                fprintf(stderr, "SSL_write() failed\n");
                exit(1);
            }
